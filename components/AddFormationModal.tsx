...
interface AddFormationModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: { nom: string; dept_id: number; nb_modules: number }) => Promise<void>;
}

export default function AddFormationModal({ isOpen, onClose, onSubmit }: AddFormationModalProps) {
    const [name, setName] = useState('');
    const [deptId, setDeptId] = useState('');
    const [nbModules, setNbModules] = useState('');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const nbModulesNum = parseInt(nbModules);
        if (!nbModules || nbModulesNum < 1) {
            setError('Number of modules must be a positive number');
            return;
        }
        ...
        try {
            await onSubmit({
                ...
                nb_modules: nbModulesNum
            });
            resetForm();
            onClose();
        } catch (err: any) {
            setError(err.response?.data?.message || 'Failed to add formation');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xl font-bold text-white">Add New Formation</h2>
                        <button
                            onClick={handleClose}
                            disabled={loading}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    {fetchingData ? (
                        ...
                        <>
                            {/* Formation Name */}
                            <div>
                                <label htmlFor="formation-name" className="block text-sm font-medium text-slate-700 mb-2">
                                ...
                                <input
                                    ...
                                    placeholder="e.g., Licence 1 - Computer Science"
                                    disabled={loading}
                            ...
                            {/* Number of Modules */}
                            <div>
                                <label htmlFor="nb-modules" className="block text-sm font-medium text-slate-700 mb-2">
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all disabled:bg-slate-50 disabled:text-slate-500"
                                />
                                <p className="mt-1 text-xs text-slate-500">Total number of modules in this formation</p>
                            </div>
