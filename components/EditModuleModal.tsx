...
interface EditModuleModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (id: number, data: { nom: string; credits: number; formation_id: number; pre_req_id?: number }) => Promise<void>;
    module: Module | null;
}

export default function EditModuleModal({ isOpen, onClose, onSubmit, module }: EditModuleModalProps) {
    ...
    useEffect(() => {
        if (isOpen && module) {
            setName(module.nom);
            setCredits(module.credits.toString());
            setFormationId(module.formation_id.toString());
            setPreReqId(module.pre_req_id?.toString() || '');
            fetchData();
        }
    }, [isOpen, module]);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setModules(modulesRes.data.filter((m: Module) => m.id !== module?.id)); // Exclude current module from prerequisites
        } catch (err) {
